...
# This script evaluates the impact of different Compute Instance (CI) sizes on performance and system metrics.
# It uses the largest possible GPU Instance (GI) and varies the CI size within it.
# Benchmarks like Blender, HPCG, LLaMA, and YOLO are run, while metrics from DCGM, IPMI, and CPU are collected.
...
def select_gi_and_launch(mig_wrapper, docker_wrapper, monitors_wrapper, suitable_gpus):
    ...
    gi_profile = gi_profile_list[-1] # We take the largest GI
    ...
    select_ci_and_launch(mig_wrapper, docker_wrapper, monitors_wrapper, suitable_gpus, gi_profile['name'])

#############################
# Select a Compute instance #
#############################
def select_ci_and_launch(mig_wrapper, docker_wrapper, monitors_wrapper, suitable_gpus, gi_profile):

    # First, we have to create a mig to retrieve the associate list of CI profile
    mig_wrapper.create_gpu_instance(gpu_id=suitable_gpus[0], gi_profiles=gi_profile)
    ...
    ci_profile = ci_profile_list[0] # We take the smallest CI

    # I) Create GIs on all GPUs
    for suitable_gpu in suitable_gpus:
        mig_wrapper.create_gpu_instance(gpu_id=suitable_gpu, gi_profiles=gi_profile) # Create MIG instance

    # II) iterate_on_complements
    iterate_on_complements(mig_wrapper, docker_wrapper, monitors_wrapper, suitable_gpus, list_gi_active[0]['name'], ci_profile['name'])

    # III) Destroy all CIs
    for suitable_gpu in suitable_gpus:
            mig_wrapper.destroy_compute_instance(gpu_id=suitable_gpu) # Not needed in theory
            mig_wrapper.destroy_gpu_instance(gpu_id=suitable_gpu)

#########################
#Complementary instances#
#########################
def iterate_on_complements(mig_wrapper, docker_wrapper, monitors_wrapper, suitable_gpus, gi_profile, ci_profile):
    index=0
    while True:
        index+=1
        stop_signal = False
        # Test if we can create a new CI on all GPUs
        for suitable_gpu in suitable_gpus:
            gi = [gi for gi in mig_wrapper.list_gpu_instance_active(gpu_id=suitable_gpu) if gi['name'] == gi_profile][0] # There is only one GI per GPU
            ci = [ci for ci in mig_wrapper.list_compute_instance_profiles(gpu_id=suitable_gpu, gi_id=gi['gi_id']) if ci['name'] == ci_profile][0] # only one profile
            if ci['free_instances'] > 0:
                mig_wrapper.create_compute_instance(gpu_id=suitable_gpu, gi_id=gi['gi_id'], ci_profiles=ci['name'])
            else:
                stop_signal = True
                break
...
if __name__ == "__main__":
    ...
    monitors_wrapper = MonitorWrapper(monitors=monitors, output_file='measures-perf-mig-ci.csv')
